using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace net.graphicintegrity.battlepets.framework
{
    public static class Constants
    {
        public const string PET_INSTANCES_XML = "../../Data/PetInstances.xml";
        public const string DAMAGE_TYPES_XML = "../../Data/DamageTypes.xml";
        public const string PET_SKILLS_XML = "../../Data/PetSkills.xml";
        public const string PETS_XML = "../../Data/Pets.xml";
        public const string PET_TYPES_XML = "../../Data/PetTypes.xml";

        public const string EFFECTIVE = "effective";
        public const string NOT_EFFECTIVE = "not effective";
        public const string SUPER_EFFECTIVE = "super effective";

    }
}
